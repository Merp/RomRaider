...
public class RomChecksum {
    ...
    public static void calculateRomChecksum(byte[] input, int storageAddress, int dataSize) {
        for (int i = storageAddress; i < storageAddress + dataSize; i+=12) {
            byte[] newSum = calculateChecksum(input,
                    (int)parseByteValue(input, Settings.ENDIAN_LITTLE, i  , 4, true),
                    (int)parseByteValue(input, Settings.ENDIAN_LITTLE, i+4, 4, true));
    ...
    public static int validateRomChecksum(byte[] input, int storageAddress, int dataSize) {
        ...
        int j = 0;
        for (int i = storageAddress; i < storageAddress + dataSize; i+=12) {
            int startAddr = (int)parseByteValue(input, Settings.ENDIAN_LITTLE, i  , 4, true);
            int endAddr   = (int)parseByteValue(input, Settings.ENDIAN_LITTLE, i+4, 4, true);
            int diff      = (int)parseByteValue(input, Settings.ENDIAN_LITTLE, i+8, 4, true);
            if (j == 0 &&
                    startAddr == 0 &&
                    endAddr   == 0 &&
    ...
    private static int validateChecksum(byte[] input, int startAddr, int endAddr, int diff) {
        ...
        for (int i=startAddr; i<endAddr; i+=4) {
            byteSum += (int)parseByteValue(input, Settings.ENDIAN_LITTLE, i, 4, true);
        }
    ...
    private static byte[] calculateChecksum(byte[] input, int startAddr, int endAddr) {
        ...
        return parseIntegerValue((Settings.CHECK_TOTAL - byteSum), Settings.ENDIAN_LITTLE, 4);
    }
}